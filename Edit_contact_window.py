...
############################## Database Connection #######################################    
def Edit_Contact_GUI(First_Name,Last_Name,Email_id,Phone_number,ID):
    ...
    def Create_connection():
        ...
        try:   
            ...
            c.execute("Update Contacts set Date_Time=?,First_Name=?, Last_Name=?, Email_id=?, Phone_Number=? where name_id=?",(date_time,First_Name,Last_Name,Email_id,Phone_number,ID))
            conn.commit()
            c.close()
            conn.close()
        except Exception as e:
            mb.showinfo("Error",'Database is not storing data'+str(e),parent=r)
            print(e)
    ...
    def submit():    
        ...
        if len(First_Name)>1:
                if len(Last_Name)>0:
                    if len(Email_id)>1:
                        if Phone_number.isdigit():
                            mb.showinfo("Success","Contact"+" "+First_Name+" "+Last_Name+" Updated Successfully\n\n Click on Refresh Button ",parent=r)
                            Create_connection()
                            e1.delete(0,tk.END)
                            e2.delete(0,tk.END)
                            e3.delete(0,tk.END)
    ...
    r.resizable(0, 0) 
    r.title('Edit Contacts')
    r.iconbitmap( r'D:\Nishu_works\Last_Year_projects\Contact_GUI\contact_icon.ico') 
    tk.Label(r, text="Edit Contact",bg='steelblue1',fg="blue",font=("Helvetica", 16,"bold"),width=30,height=1,anchor='center').pack()
